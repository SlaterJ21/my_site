...
import '../../../../css/about/spotify/sizes/small.css'
import { connect } from 'react-redux'
import { bindActionCreators } from 'redux'
import { changeView } from '../../../../actions'

class SpotifySmall extends Component {
...
componentDidMount() {
  ...
  render() {
    return (
      <Row className="center-fix">
        <SmallAboutButtons/>
        <div className="spotifyContainer-s">
          <Spotify props={this.state.spotify[this.state.spotifyIndex][0]}/>
          <Col s={3}></Col>
          <Col s={3}>
          <div className="nextAlbum-s button-right-s" onClick={() => this.prevDataSet()}><Icon small>chevron_left</Icon></div>
          </Col>
          <Col s={3}>
          <div className="nextAlbum-s button-left-s right-align" onClick={() => this.nextDataSet()}><Icon small>chevron_right</Icon></div>
          </Col>
        </div>
      </Row>
    );
