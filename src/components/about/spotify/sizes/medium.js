...
import '../../../../css/about/spotify/sizes/medium.css'
import { connect } from 'react-redux'
import { bindActionCreators } from 'redux'
import { changeView } from '../../../../actions'

class SpotifyMedium extends Component {
  ...
  render() {
    return (
      <Row>
        <MediumAboutButtons/>
        <Col m={2}>
          <div className="nextAlbum-m button-right-m right-align" onClick={() => this.prevDataSet()}><Icon small>chevron_left</Icon></div>
        </Col>
        <Col m={8} className="spotifyContainer-m center-align">
          <Spotify props={this.state.spotify[this.state.spotifyIndex][0]}/>
        </Col>
        <Col m={2}>
          <div className="nextAlbum-m button-left-m" onClick={() => this.nextDataSet()}><Icon small>chevron_right</Icon></div>
        </Col>
      </Row>
    );
  }
